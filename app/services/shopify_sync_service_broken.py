"""
Background service for syncing Shopify orders automatically (NO AUTH)
"""
import asyncio
import logging
...
class ShopifySyncService:
    ...
    async def _sync_all_users(self):
        """Sync orders for all users with Shopify configuration (simplified for no-auth)"""
        try:
            # Just try to sync the dummy user
            await self._sync_user_orders("dummy_user_123")
    ...
    async def _sync_user_orders(self, user_id: str) -> Dict:
        ...
        try:
            ...
            if not config_result["success"]:
                logger.info(f"No Shopify config found for user {user_id}")
            ...
            error_msg = str(e)
            logger.info(f"Sync attempt for user {user_id}: {error_msg}")
