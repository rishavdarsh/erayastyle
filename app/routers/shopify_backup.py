"""
Shopify router for order management and analytics with real API integration
"""
...
async def save_config(
    ...
    try:
        ...
        # Save configuration
        result = save_shopify_config(current_user["id"], shop_domain, access_token)
...
async def get_config(
    ...
    """Get Shopify configuration (without sensitive data)"""
    try:
        result = get_shopify_config(current_user["id"])
...
async def test_connection(
    ...
    try:
        config_result = get_shopify_config(current_user["id"])
        if not config_result["success"]:
...
async def manual_sync(
    ...
    try:
        ...
        background_tasks.add_task(sync_shopify_orders, current_user["id"])
...
async def get_orders(
    ...
    """Get synced Shopify orders from database"""
    try:
...
async def get_analytics(
    ...
    """Get order analytics from synced data"""
    try:
