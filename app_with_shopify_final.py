...
from utils.shopify_config import (
    load_shopify_config, 
    save_shopify_config, 
    get_shopify_config_for_display,
...
def eraya_shopify_settings_page(request: Request, current_user: Dict = Depends(require_roles("owner", "admin"))):
    config = get_shopify_config_for_display()
...
# ==================== SHOPIFY CONFIGURATION API ====================

@app.post("/api/shopify/save-config")
async def save_shopify_configuration(request: Request, current_user: Dict = Depends(require_roles("owner", "admin"))):
    """Save Shopify configuration to secrets file"""
    try:
        data = await request.json()
        ...
        # Save configuration
        success, message = save_shopify_config(shop_domain, access_token)
...
@app.get("/api/shopify/test-connection")
def test_shopify_connection(current_user: Dict = Depends(require_roles("owner", "admin"))):
    """Test connection to Shopify using saved credentials"""
    try:
        config = load_shopify_config()
        shop = config.get("SHOPIFY_SHOP")
        ...
        # Test connection by calling Shopify's shop.json endpoint
        ...
        if response.status_code == 200:
            ...
            return JSONResponse({
                "success": False,
                "error": "Shop not found - check your shop domain"
            })
        else:
            return JSONResponse({
                "success": False,
        ...
        return JSONResponse({
            "success": False,
            "error": "Could not connect to Shopify - check your internet connection"
        })
    except Exception as e:
        return JSONResponse({
            "success": False,
...
@app.get("/api/shopify/config-status")
def get_shopify_config_status(current_user: Dict = Depends(require_roles("owner", "admin"))):
    """Get current Shopify configuration status"""
    try:
        config = get_shopify_config_for_display()
        return JSONResponse({
...
# Initialize Shopify configuration on startup
@app.on_event("startup")
async def startup_event():
    """Load Shopify configuration on application startup"""
    config = load_shopify_config()
    if config["SHOPIFY_SHOP"] and config["SHOPIFY_TOKEN"]:
        print(f"✅ Shopify configured for shop: {config['SHOPIFY_SHOP']}")
    else:
        print("⚠️  Shopify not configured. Visit /shopify/settings to configure.")
